...
static auto pci_dev_get_cfg(fs::path &pci_dev_entry)
{
    ...
    return std::make_pair(std::move(ptr), cfg_size == pci_cfg_legacy_len ?
                                          cfg_space_type::LEGACY : cfg_space_type::ECS);
}
...
void PCITopologyCtx::populate()
{
    ...
    for (const auto &pci_dev : fs::directory_iterator {sys_pci_devs_path}) {
        auto entry = fs::absolute(pci_dev);
        ...
        if (!res || mres.size() < 5) {
            ...
            auto [data, cfg_type] = pci::pci_dev_get_cfg(entry);
            devs_.emplace_back(dom, bus, dev, func, cfg_type, std::move(data));
        }
    }
}
