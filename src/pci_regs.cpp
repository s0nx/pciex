                          ...
                          const RegLinkCap2 &link_cap2)
{
    ...
    if (tgt_vt_bit > (supported_link_speed_vt.size() - 1))
        return std::format(" < reserved Link Speeds Vector bit pos ({})>", tgt_vt_bit);

    if (!supported_link_speed_vt[tgt_vt_bit])
        return std::format(" [{}] {} link speed is not supported by port",
                           tgt_vt_bit, LinkSpeedBitDesc(tgt_vt_bit));
    else
        return std::format(" {} link speed: {}",
...
std::string CmplTimeoutRangesDesc(const RegDevCap2 &dev_cap2)
{
    ...
    if (ranges_supported.none())
        ...
        return std::format("A(50us-10ms)[{}] B(10ms-250ms)[{}] C(250ms-4s)[{}] D(4s-64s)[{}]",
                           ranges_supported[0] ? '+' : '-',
                           ranges_supported[1] ? '+' : '-',
                           ranges_supported[2] ? '+' : '-',
                           ranges_supported[3] ? '+' : '-');
...
std::string SuppLinkSpeedDesc(const uint8_t link_speed_vector)
{
    ...
    return std::format("2.5GT/s[{}] 5GT/s[{}] 8GT/s[{}] 16GT/s[{}]",
